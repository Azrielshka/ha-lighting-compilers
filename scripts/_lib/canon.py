...
# (решение владельца 2026-07-20, допущения приняты явно).
#
# ⚠ Техгруппы (`tech_group_*`) в неё НЕ входят. Они ПОДМНОЖЕСТВО этажных: одно
# и то же техпомещение состоит и там, и там. Возьми кто-нибудь «все группы из
# файла» скопом — техпомещения посчитались бы дважды и перекосили яркость.
#
# ⚠ Яркость получается СРЕДНИМ ОТ СРЕДНИХ по этажам, а не средним по лампам:
...
# бейджем, где ползунка нет, — поэтому цена допущения невелика. Понадобится
# честная яркость — состав придётся сделать плоским, из общих групп помещений.
#
# ⚠ Помещение без этажа в группу объекта не попадёт: оно не попало ни в одну
# этажную. Это видно в отчёте шага — generate_floor_groups печатает про такие
# помещения предупреждение.

def object_group_name() -> str:
    """«Весь объект» — отображаемое имя группы всего здания."""
    return "Весь объект"


def object_group_unique_id() -> str:
    """object_all — ключ в реестре HA. НЕ entity_id, см. object_light_entity."""
    return "object_all"


def object_light_entity() -> str:
    """light.ves_obekt — то, как HA назовёт группу всего объекта.

    Как и у этажных: entity_id выводится из ИМЕНИ через slugify, а unique_id
    только регистрирует запись в реестре. `light.object_all` не существует.
    """
    return f"{HA_LIGHT_DOMAIN}.{slugify_room(object_group_name())}"
...
def tech_light_entity(floor: int) -> str:
    ...
    ⚠ В Area НЕ назначается (решение владельца 2026-07-22): в Area этажа уже
    лежит floor_light_entity, а инвариант «ровно одна световая сущность в Area»
    нарушать нельзя — см. ba_labels() ниже. Отдельную Area под тех.помещения
    решили не заводить: управление ими по расписанию не требуется.
    """
    return f"light.{slugify_room(tech_group_name(floor))}"


# ============================================================
# МЕТКИ ДЛЯ ОРКЕСТРАТОРА ЗДАНИЯ (Building Automation)
# ============================================================
#
# Оркестратор — отдельная кастомная интеграция, которая занимается режимами
# здания и в рантайме рассылает команды по Areas. Мы для неё размечаем реестр:
# он читает метки, а не наши файлы. Контракт — docs/internal/contract-*.md.
#
# ⚠ Имена меток ASCII не по стилю, а по необходимости: label_id HA выводит из
# имени через slugify, и от кириллицы он был бы непредсказуем. Имена жёстко
# зашиты в Оркестраторе — менять их односторонне нельзя.
#
# ⚠ Инвариант, ради которого всё затевалось: в Area ровно ОДНА сущность домена
# light. Сервисный вызов с целью area_id разворачивается во все световые
# сущности Area; будь там и лампы, и группа — каждая лампа получила бы две
# команды, одну напрямую и одну через группу. Это удвоило бы трафик на шине
# DALI, который весь проект старательно экономит.

BA_FLOOR_AREA_LABEL: str = "ba_floor_area"

BA_TYPE_LABEL_PREFIX: str = "ba_type_"


def ba_type_label(space_type: str) -> str:
    """korridor -> ba_type_korridor — метка типа помещения на Area.

    Нужна, чтобы Оркестратор мог задать профиль режима сразу всем помещениям
    одного типа («во всех коридорах на Перемене свет 100 %»). Без меток
    инженер настраивал бы сорок помещений поштучно.
    """
    return f"{BA_TYPE_LABEL_PREFIX}{str(space_type).strip().lower()}"


def ba_labels() -> list[str]:
    """Все метки, которые проставляет генератор: этажная + по типу помещения.

    Порядок фиксирован (этажная первой, типы — как в ALLOWED_SPACE_TYPES,
    отсортированные), чтобы дифф деплоя не плясал между запусками.
    """
    return [BA_FLOOR_AREA_LABEL] + [
        ba_type_label(t) for t in sorted(ALLOWED_SPACE_TYPES)
    ]
...
# ------------------------------------------------------------
# ⚠ ВРЕМЕННОЕ: второй фильтр тем же набором типов, но выпадающим списком
# ------------------------------------------------------------
# Заведён 2026-07-20 по просьбе владельца, чтобы СРАВНИТЬ два вида управления на
# живом объекте: семь плиток против одного селекта. Постоянного места в проекте
# у него нет — после сравнения один из двух уходит.
#
# ⚠ Селект по определению ОДНОЗНАЧЕН: «Коридоры + Залы» одновременно он не
# умеет. Это и есть предмет сравнения — стоит ли мультивыбор своего места на
# экране.
#
# ⚠ Как два фильтра уживаются. Условия видимости верхнего уровня складываются
# по И, поэтому карточка видна, только если её пропустили ОБА. Каждый глушится
# своим «Все»:
#     булевы на «Все»  -> рулит селект
#     селект на «Все»  -> рулят плитки
#     оба сужены       -> пересечение (предсказуемо, но для сравнения бесполезно)
#
# Убирая проигравший вариант, не забудьте снять и второе условие в
# build_compact_card — иначе карточки останутся привязаны к сущности, которой
# больше нет, и этаж окажется пустым.
NAV_PICK_ID: str = "nav_type_pick"


def nav_pick_entity() -> str:
    """input_select.nav_type_pick — тот же фильтр, но одним списком."""
    return f"input_select.{NAV_PICK_ID}"


def nav_pick_options() -> list:
    """Опции списка: «Все помещения» первой, дальше типы в порядке канона.

    Строки обязаны совпасть с NAV_TYPE_LABELS символ в символ: по ним же
    строится условие видимости карточки. Разъедутся — фильтр молча перестанет
    показывать помещение.
    """
    return [NAV_TYPE_ALL_LABEL] + list(NAV_TYPE_LABELS.values())
