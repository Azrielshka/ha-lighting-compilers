...
# "ИНФ контроллеры", "Глоссарий" — их пайплайн не трогает.
SHEET_NAME: str = "Проектная БД"

# Лист связей датчиков для zone_manager.json. Опциональный: на объекте без
# него zone_manager просто не собирается. Ищется по имени с обрезкой пробелов
# (заголовок гуляет между "Группы соседей" и "Группы соседей ").
NEIGHBORS_SHEET: str = "Группы соседей"

# Колонки листа «Группы соседей» (по позиции слева направо).
NEIGHBORS_COLUMNS = (
    "Основной датчик",
    "Основная группа света",
    "Соседние датчики",
    "Соседние группы света",
    "Дальние соседний датчики",
)
