...
# 3. Установка и запуск на новой машине

Нужен Python 3.10–3.12 с галочкой **Add Python to PATH**.
...
⚠ Именно `.venv\Scripts\python.exe -m pip`, а не голый `pip`: иначе пакеты
уедут в другое окружение, и лаунчер их не найдёт.

Лаунчер (GUI) — обычный способ работы:

```bash
python launcher/main.py
```

Тот же пайплайн из CLI:
...
# ... остальные генераторы, см. bash_command.txt
...
Полная шпаргалка команд — `bash_command.txt`.
