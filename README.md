...
| 7 | `generate_helpers.py` | вспомогательные объекты → `lighting-compilers.yaml` |
...
Деплой перезаписывает только своё и не трогает файлы наладчика — гарантия в
узнаваемом имени: префикс `zm_` либо явное исключение (`lighting-compilers.yaml`,
пакет помощников). Рестарт HA деплой **не делает** — его выполняет человек.
