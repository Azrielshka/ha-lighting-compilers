...
class LauncherWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.append_log("Step 10: config persistence is enabled")
    ...
    def _connect_signals(self) -> None:
        ...
        #
        # Зачем это нужно:
        #   если пользователь меняет путь руками, а не через Browse,
        #   изменения тоже должны попасть в config.
        # ------------------------------------------------------------
    ...
    def _restore_saved_or_default_config(self) -> None:
        ...
        # ------------------------------------------------------------
        # Если путь к проекту из config существует — используем его
        # ------------------------------------------------------------
        ...
        # ------------------------------------------------------------
        # Excel-path из config восстанавливаем только если:
        #   - он не пустой
        #   - файл существует
        #
        # Если файл не существует, просто пропускаем его и не ломаем запуск.
        # ------------------------------------------------------------
        ...
        # ------------------------------------------------------------
        # Если ничего не загрузили из сохранённого config,
        # пытаемся использовать startup defaults
        # ------------------------------------------------------------
        if not loaded_from_saved:
            self._apply_startup_defaults()

        # ------------------------------------------------------------
        # После восстановления сразу сохраняем актуальное состояние.
        #
        # Это полезно, например, если старый config был частично битый,
        # а launcher подставил defaults.
        # ------------------------------------------------------------
    ...
    def _validate_runtime_config(self) -> dict | None:
        ...
        # ------------------------------------------------------------
        # Excel проверяем только если поле заполнено.
        #
        # Зачем так:
        #   пользователь может намеренно оставить поле пустым,
        #   чтобы normalize_excel.py использовал свой внутренний
        #   DEFAULT_EXCEL_PATH.
        # ------------------------------------------------------------
