...
Оверхол карточек Главной и этажей + освещённость в карточках рекреации/класса.
Требует деплоя (новые группы света, helpers, карточки).

### Added

- **Группы этажа по типу** `light.<этаж>_<тип>` (`generate_floor_groups`) —
...
- **Главная**: тех.плитка → тумблер автоматики этажа (свет и switch разными
  строками, `features_position: inline`); секции ×3, карточки `columns: 9`;
  сервис +2 заглушки (`problemy`, `monitoring`); бейджи `center/bottom/scroll`;
  нумерация навигации по возрастанию.
- **Этажи**: бейджи `center/bottom/scroll`; карточка помещения `inline` +
...
Интеграция с Оркестратором здания и генератор JSON Zone Manager. Формат таблицы
расширен листом «Группы соседей» (опционален: без него собирается всё, кроме
zone_manager), состав пайплайна вырос до 10 шагов.

⚠ **После деплоя обязателен рестарт Home Assistant** — появились новые помощники
(`vacant_delay_<тип>`), новая разметка реестра и `zone_manager.json`. И четыре
шага на объекте после обновления с 3.1.0:

1. **Убрать вручную** осиротевшие `input_boolean.regim_auto_<N>` — генератор их
   больше не создаёт, но и не удаляет.
2. **Выставить задержки гашения** `input_number.vacant_delay_<тип>`: у них нет
   `initial`, на первом старте каждая = 0 (свет гаснет мгновенно).
3. **Вызвать `zone_manager.reload`** после деплоя — деплой сервисы не дёргает,
   иначе новый конфиг зон не вступит в силу.
4. **Оркестратор здания должен быть установлен** — на его сущности теперь
   ссылаются гейт датчиков и бейджи режима этажей.

### Added

- **Zone Manager**: шаг 10 пайплайна `generate_zone_manager.py` собирает
  `zone_manager.json` из листа «Группы соседей» (forward-fill, заглушки,
  валидация пусто/маркер, битые ссылки). Деплой → `/config/zone_manager.json`,
  кнопка `[10] ZONE MANAGER` в лаунчере. Закрыт долг «свет по датчикам не
  включается без JSON Zone Manager».
- **Оркестратор здания**: гейт датчиков в blueprint'ах (fail-open шаблоном),
  разметка Areas (метки `ba_floor_area` / `ba_type_*`, назначение света),
  бейджи режима на сущности Оркестратора. Разметка, гейт и бейджи приняты на
  объекте.

### Changed

- `input_number.vacant_delay` → по одному на тип помещения
  (`vacant_delay_<тип>`), без `initial`: значение принадлежит объекту.
- `input_boolean.regim_auto_<N>` больше не создаётся (заменён сущностями
  Оркестратора в бейджах).

---

## [3.1.0] - 2026-07-20

Навигация: фильтр помещений по типу и группа света всего объекта. Совместимо с
3.0.0 — формат таблицы и состав пайплайна не менялись.

⚠ **Требует рестарта Home Assistant после деплоя:** появились новые помощники и
новая группа света, а это YAML-пакеты. Без рестарта фильтр не заработает, а
карточки помещений не пройдут условие видимости — этажные страницы окажутся
пустыми.

### Added

- **Фильтр навигации по типу помещения.** На этажных страницах появилась панель
  «Показывать»: «Все помещения» и шесть типов. Отметили «Коридоры» — из
  девятнадцати карточек остались коридоры; отметили ещё «Залы» — видно оба типа
  сразу. Помощники ОБЩИЕ на объект, поэтому фильтр не сбрасывается при переходе
  между этажами.

  Реализован нативно, без зависимостей: семь `input_boolean` и `visibility` у
  карточек помещений. Своя JS-карта рассматривалась и отклонена — задача её не
  требует; довод «фильтр это состояние вида, а не объекта» записан в
  `decisions.md` вместе с ценой отказа (фильтр общий для всех, кто смотрит
  дашборд).

  ⚠ У `input_boolean.nav_type_all` стоит `initial: true`. Без него после
  первого старта HA все фильтры выключены и этажные страницы оказываются
  пустыми.

- **Временно: второй фильтр списком** (`input_select.nav_type_pick`) — тот же
  набор типов одним выпадающим списком, рядом с плитками. Заведён, чтобы
  сравнить два вида управления на живом объекте: плитки занимают ~200px, список
  ~56px, но список однозначен и «Коридоры + Залы» одновременно не умеет. После
  сравнения один из двух уходит.

- **Группа «Весь объект»** (`light.ves_obekt`) и бейдж управления светом здания
  на Главной. Группы, объединяющей этажи, не было — включить свет целиком было
  нечем.

  Состав **вложенный**, из групп этажей. ⚠ Отсюда три принятых допущения:
  яркость считается как среднее от средних по этажам (вкл/выкл при этом точен,
  а бейдж ползунка не имеет); техгруппы в состав не входят, иначе техпомещения
  учлись бы дважды; помещение без этажа не попадёт вообще. Подробности и цена
  каждого — в `decisions.md`.

  Попутно: `badges_position` и `badges_wrap` в шапке Главной стояли вхолостую —
  настройки раскладки были, а самого списка `badges` не было.

- `tests/test_object_group.py` — 10 тестов на состав, иерархию и капкан
  `unique_id` ≠ `entity_id`.

- `tests/test_nav_filter.py` — 15 тестов, включая вычислитель `visibility` по
  правилам HA. Без него фильтр проверялся бы только глазами на объекте, а
  каждая проверка — деплой с рестартом.

### Changed

- Главная страница **не изменилась** (решение владельца): помощники общие, и
  фильтр с этажной страницы действует везде. Главная отвечает на вопрос «на
  какой этаж пойти», а не «какие тут коридоры».
