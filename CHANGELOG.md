...
- **Шапка и бейджи этажного view** — шаблон `templates/lovelace/floor/view.yaml`
  ...
  бейдж света этажа (`light.floor_<N>_all` — наша же группа), бейдж режима
...
- **Деплой карточек** — цель `lovelace` в `deploy.py`, канал WebSocket:
  ...
  ручные views владельца сохранить, вставить сразу после первого view) →
  записать. Дашборд остаётся UI-редактируемым, рестарт не нужен.
...
- **Тесты** — 378, включая приёмочные на реальной фикстуре и проверку
...
заводит вручную — генератор их не создаёт, только ссылается.
